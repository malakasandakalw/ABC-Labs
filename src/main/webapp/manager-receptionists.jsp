...
<title>Manager Receptionists</title>
<link
	...
	<div class="container">
		<p>${message}</p>
         <div class="d-flex align-items-center mb-3">
            <div class="me-auto">
               <h3 class="title">Receptionists</h3>
            </div>           
            <div class="ms-auto">
				<a class="btn btn-primary" href="manager-add-new-receptionist.jsp">Create Receptionist</a>
            </div>
         </div>
         <hr>
		...
		<table class="table table-stripped" id="dataTable">
			...
			<tbody>
				<tag:forEach var="receptionist" items="${receptionistsList}">
					<tr class="filter">
						<td>${receptionist.name}</td>
						<td>${receptionist.email}</td>
						<td>${receptionist.isActive == 1 ? 'Active' : 'Inactive'}</td>
						<td>
							<div class="action-div">
								<form method="get" action="receptionists">
								   <input type="hidden" name="receptionist_id" value="${receptionist.id}" required>
